...
st.set_page_config(page_title="In Tem QR Chuong Duong - Final", page_icon="📱")
...
st.info("Cấu trúc QR: Booking-MãKho-MãST-SốKiện-SốHD (Không chứa dữ liệu K)")

uploaded_file = st.file_uploader("Tải file Excel (Cấu trúc mới nhất)", type=['xlsx'])

if uploaded_file:
    try:
        # Đọc dữ liệu, làm sạch tên cột để tránh khoảng trắng dư thừa
        ...
        if st.button("🚀 XUẤT PDF TẤT CẢ TEM"):
            ...
            for _, row in df.iterrows():
                # Lấy dữ liệu theo tên cột chính xác trong ảnh của bạn
                ...
                # TẠO NỘI DUNG QR ĐỒNG NHẤT (Bỏ dữ liệu K)
                ...
                # Vòng lặp in số lượng tem tương ứng với số kiện
                for i in range(1, tong_so_kien + 1):
                    ...
                    # Hiển thị số thứ tự kiện to ở giữa (Ví dụ: 1 / 10)
                    ...
                    # Hiển thị chuỗi text dưới QR để đối chiếu
            ...
            c.save()
            st.download_button("📥 TẢI TEM PDF", buffer.getvalue(), f"Tem_QR_MT_Final_{datetime.now().strftime('%d%m')}.pdf")
